...
const Products = () => {
      ...
      const formattedProducts = await Promise.all(
        data?.map(async (product) => {
          ...
          const validImages = images.filter(
            (img) => img.image_url && !img.image_url.startsWith("blob:")
          );
          const firstImage =
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orbio-light to-white">
        <Header />
        <div className="container mx-auto px-4 py-8">
          <Card className="text-center py-12 border-0 shadow-xl bg-white/90 backdrop-blur-sm">
            <CardContent>
              <h2 className="text-2xl font-bold mb-4 text-orbiomary">
                Acesso Restrito
              </h2>
              <p className="text-gray-600">
                Você precisa estar logado para ver os produtos.
              </p>
            </CardContent>
          </Card>
        </div>
  ...
  if (shouldShowLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orbioht to-white">
        <Header />
        <div className="container mx-auto px-4 py-8">
          <div className="mb-8">
            <h1 className="text-4xl font-bold text-orbiomary mb-6">
    ...
    <div className="min-h-screen bg-gradient-to-br from-orbioht to-white">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          {/* Barra de busca */}
          <div className="relative mb-6">
            <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-orbiomary h-5 w-5" />
            <Input
              type="text"
              placeholder="Buscar produtos, materiais ou descrições..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="pl-4 h-12 border-2 border-orbioht focus:border-orbiorbio rounded-xl bg-white/80 backdrop-blur-sm shadow-lg transition-all duration-300 focus:shadow-xl"
            />
          </div>
        ...
        {filteredProducts.length === 0 && !isLoading && (
          <Card className="text-center py-16 border-0 shadow-xl bg-white/90 backdrop-blur-sm">
            <CardContent>
              ...
              <h3 className="text-2xl font-bold mb-4 text-orbiomary">
                {showFavorites
                  ? "Nenhum produto favoritado"
                  : "Nenhum produto disponível ou encontrado com os filtros"}
              </h3>
